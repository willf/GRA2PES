...
month.list        <- c(12) ###CHANGE
dow.list          <- c("weekdy","satdy","sundy")

#Set observation files and directories
base.in     <- paste("/wrk/csd4/clyu/GHG_CO2/Improving_inventory/V7_GRA2PES2021/scal_sum_ncf/base2021mm_EGUonly_rds/point/Month12",sep="") ###CHANGE
    ...
    for(v in 1:length(var.list))
    {
      ...
      for(s in 1:length(sector.list))
      {
        ...
        if(length(emis.file) > 0)
        {
          ...
          #Do not scale CEMS data, they are already the target year and month
