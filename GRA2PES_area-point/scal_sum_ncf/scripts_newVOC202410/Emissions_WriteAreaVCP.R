...
og.list      <- c("OnG")
fug.list     <- c("OffG","EVAPGAS","STORAGE","CONST","DUST")
...
#Exclude OnG, EVAPGAS, Mobile, VCP, AgBURN in area total
#sector.list <- c(comm.list, res.list, indf.list, indp.list, "OffG", "STORAGE", "CONST", "DUST", "CROP", "LIVESTOCK", waste.list)
sector.list  <- c(vcp.list)
...
crit.list         <- c("CO","NH3","NOX","PM10-PRI","PM25-PRI","SO2") #Exclude VOC
