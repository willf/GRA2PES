...
#Set directories
#input.dir    <- "/nfs/csd4/wrf-chem/bmcdonald/COVID_Modeling/CSD_Emissions/AREA19/input"
input.dir <- "/wrk/charkins/emissions/GRA2PES/V7_NRT_scaling/Cooking21"
#input.dir    <- paste("/wrk/csd4/bmcdonald/COVID_Modeling/CSD_Emissions/AREA",sprintf("%02d",year-2000),"/input",sep="")
#output.dir   <- paste("/wrk/csd4/bmcdonald/COVID_Modeling/CSD_Emissions/AREA",sprintf("%02d",year-2000),sep="")
output.dir   <- "/wrk/users/charkins/emissions/V7_GRA2PES/Cooking21_newVOC202410"


out.name     <- "Cooking_Mean"
...
gdp.file     <- paste("AreaCooking","_monthly.csv",sep="")

#VCP use and emission factors
sector.list  <- c("Cooking_Mean")
sector.ind   <- c(FALSE)

voc.use      <- c(15.70744*1.601475999)/1000 #kg/person/d (US)

voc.ef.us    <- c( 1000) #g/kg (US)
voc.ef.ca    <- c( 1000) #g/kg (CA)
voc.ef.la    <- c( 1000) #g/kg (LA)
voc.ef.ny    <- c( 1000) #g/kg (NY)
voc.ef.nj    <- c( 1000) #g/kg (NJ)
voc.ef.ct    <- c( 1000) #g/kg (CT)
voc.ef.nyc   <- c( 1000) #g/kg (NYC)
...
voc.wt.file  <- "areaCooking_wt_202410.csv"
voc.mw.file  <- "areaCooking_mw_202410.csv"

#Diurnal files (number of diurnal profiles must match number of sectors)
diurnal.file   <- "areaCooking_diurnal.csv" #local time
