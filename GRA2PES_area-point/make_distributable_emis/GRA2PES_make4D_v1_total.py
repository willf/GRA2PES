...
class inputs():
    ...
    out_file1 = '/wrk/users/charkins/emissions/V7_GRA2PES/GRA2PESv1.0/GRA2PESv1.0_total/[YYYY][MM]/[DOW]/GRA2PESv1.0_total_[YYYY][MM]_[DOW]_00to11Z.nc'
    out_file2 = '/wrk/users/charkins/emissions/V7_GRA2PES/GRA2PESv1.0/GRA2PESv1.0_total/[YYYY][MM]/[DOW]/GRA2PESv1.0_total_[YYYY][MM]_[DOW]_12to23Z.nc'
...
class out_nc_file():
    ...
    variables = {    'ffCO2':{'units':'mole km^-2 hr^-1','description':'Fossil Fuel Carbon Dioxide'},
...
def calc_fracs(wrfchemi_ds,file):
    ...
    # dhm1.to_netcdf('dhm1.nc')
    # dhm2.to_netcdf('dhm2.nc')
    # bot1.to_netcdf('bot1.nc')
    # bot2.to_netcdf('bot2.nc')
    # top1.to_netcdf('top1.nc')
    # top2.to_netcdf('top2.nc')
...
# def sum_files(files,year,month,dow,dayhalf):
    ...
    # for c,file in enumerate(files):
        ...
        # print('Summing file: ' + fn)
        
        # if c == 0: 
            # ds_out = xr.open_dataset(fn,cache=False).drop_vars(['Times'])
            # if 'ffCO2' not in ds_out.data_vars:
                # ds_out['ffCO2'] = ds_out['CO2'].copy(deep=True)
            # #breakpoint()
            
            # add_vars = set(out_nc_file.variables.keys()).difference(set(ds_out.data_vars))
            # for var in add_vars:
                # ds_out[var] = xr.zeros_like(ds_out['ffCO2'],dtype=float)
            
        # else:
            # ds = xr.open_dataset(fn,cache=False).drop_vars(['Times'])
            # if 'ffCO2' not in ds.data_vars:
                # ds['ffCO2'] = ds['CO2'].copy(deep=True)
                
            # add_vars = set(out_nc_file.variables.keys()).difference(set(ds.data_vars))
            # for var in add_vars:
                # ds[var] = xr.zeros_like(ds['ffCO2'],dtype=float)
...
def place_vars(year,month,dow,out_ds_empty,frac1,frac2,x_y):
    ...
    # #area_emis1 = xr.open_dataset(area_file1,cache=False)
    # #area_emis2 = xr.open_dataset(area_file2,cache=False)
    # #breakpoint()
    # print('Making gridded dataset 00z: ')
    # area_emis1 = sum_files(inputs.gridded_files,year=year,month=month,dow=dow,dayhalf='00to12Z')
    # #breakpoint()
    # print('Making gridded dataset 12z: ')
    # area_emis2 = sum_files(inputs.gridded_files,year=year,month=month,dow=dow,dayhalf='12to24Z')
    
    # # ADD in if ffCO2 not in then copy CO2
    
    # #point_emis1 = xr.open_dataset(inputs.point_file.replace('[YYYY]',str(year)).replace('[YY]',str(year)[2:]).replace('[MM]',str(month).zfill(2)).replace('[DOW]',dow).replace('[dayhalf]',dayhalf),cache=False)
    # #point_emis2 = xr.open_dataset(inputs.point_file.replace('[YYYY]',str(year)).replace('[YY]',str(year)[2:]).replace('[MM]',str(month).zfill(2)).replace('[DOW]',dow).replace('[dayhalf]',dayhalf),cache=False)
    
    # print('Making point dataset 00z: ')
    # point_emis1 = sum_files(inputs.point_files,year=year,month=month,dow=dow,dayhalf='00to12Z').assign_coords({'x':x_y.loc[dict(x_y='x')].drop_vars(['x_y','latlon_coord']),'y':x_y.loc[dict(x_y='y')].drop_vars(['x_y','latlon_coord'])})
    # print('Making point dataset 12z: ')
    # point_emis2 = sum_files(inputs.point_files,year=year,month=month,dow=dow,dayhalf='12to24Z').assign_coords({'x':x_y.loc[dict(x_y='x')].drop_vars(['x_y','latlon_coord']),'y':x_y.loc[dict(x_y='y')].drop_vars(['x_y','latlon_coord'])})
    
    
    
    # #point_emis_sum_row = point_emis1.sum(dim=['Time']) + point_emis2.sum(dim=['Time'])
    ...
    for c, variable in enumerate(out_nc_file.variables.keys()):
        ...
        if variable in point_emis1.data_vars:
            #emis_points1 = point_emis1[variable]*frac1*unit_conv 
            #emis_points2 = point_emis2[variable]*frac2*unit_conv
        ...
        del area_emis1,area_emis2,point_emis1,point_emis2
...
def aggregate_by_xy(emis,var,sizes):
    ...
    #e= e[e['x']<sizes['west_east']-1]]
    #e= e[e['x']>=0]
    #e= e[e['y']<sizes['south_north']-1]]
    #e= e[e['y']>=0]
    
    #emis_points_grouped = e.to_xarray()
    
    #emis_points_grouped['x'] = emis_points_grouped['x'].min(dim=['Time','bottom_top'])
    #emis_points_grouped['y'] = emis_points_grouped['y'].min(dim=['Time','bottom_top'])
    
    #emis_points_grouped = emis_points_grouped.where(emis_points_grouped['x']<sizes['west_east']-1,drop=True)
    #emis_points_grouped = emis_points_grouped.where(emis_points_grouped['x']>=0,drop=True)
    #emis_points_grouped = emis_points_grouped.where(emis_points_grouped['y']<sizes['south_north']-1,drop=True)
    #emis_points_grouped = emis_points_grouped.where(emis_points_grouped['y']>=0,drop=True)
    ...
    for year in inputs.years:
        for month in inputs.months:
            for dow in inputs.dows:
                ...
                #point_emis1 = sum_files(inputs.point_files,year=year,month=month,dow=dow,dayhalf='00to12Z')
                #point_with_xy =point_emis1.assign_coords({'x_y':x_y.drop_vars('latlon_coord')})
                #point_with_xy =point_emis1.assign_coords({'x':x_y.loc[dict(x_y='x')].drop_vars(['x_y','latlon_coord']),'y':x_y.loc[dict(x_y='y')].drop_vars(['x_y','latlon_coord'])})

                #emis_points1 = point_with_xy['CO2']*frac1
                # e = emis_points1.to_dataframe(name='CO2').reset_index()[['Time','bottom_top','x','y','CO2']].groupby(['Time','bottom_top','x','y']).sum().reset_index(['x','y'])
                
                # e['id'] = e.groupby(['x','y']).ngroup()
                # e.set_index('id',append=True,inplace=True)
                # emis_points1_grouped = e.to_xarray()
                # emis_points1_grouped['x'] = emis_points1_grouped['x'].min(dim=['Time','bottom_top'])
                # emis_points1_grouped['y'] = emis_points1_grouped['y'].min(dim=['Time','bottom_top'])
                
                #emis_points1_grouped = aggregate_by_xy(point_with_xy['CO2']*frac1,'CO2',wrfchemi_ds.sizes) # REMEMBER UNIT CONV
