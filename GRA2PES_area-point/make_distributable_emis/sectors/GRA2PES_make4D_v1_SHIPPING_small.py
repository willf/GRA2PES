...
class inputs():
    ...
    out_file1 = '/wrk/users/charkins/emissions/V7_GRA2PES/GRA2PESv1.0/GRA2PESv1.0_SHIPPING_small/[YYYY][MM]/[DOW]/GRA2PESv1.0_SHIPPING_[YYYY][MM]_[DOW]_00to11Z.nc'
    out_file2 = '/wrk/users/charkins/emissions/V7_GRA2PES/GRA2PESv1.0/GRA2PESv1.0_SHIPPING_small/[YYYY][MM]/[DOW]/GRA2PESv1.0_SHIPPING_[YYYY][MM]_[DOW]_12to23Z.nc'
...
def place_vars(year,month,dow,out_ds_empty,frac1,frac2,x_y):
    ...
    for c, variable in enumerate(out_nc_file.variables.keys()):
        ...
        if inputs.point_files:
            ...
            point_emis1 = sum_files_variable(inputs.point_files,year=year,month=month,dow=dow,dayhalf='00z',variable=variable).assign_coords({'x':x_y.loc[dict(x_y='x')].drop_vars(['x_y','latlon_coord']),'y':x_y.loc[dict(x_y='y')].drop_vars(['x_y','latlon_coord'])})
            print('Making point dataset 12z: ')
            point_emis2 = sum_files_variable(inputs.point_files,year=year,month=month,dow=dow,dayhalf='12z',variable=variable).assign_coords({'x':x_y.loc[dict(x_y='x')].drop_vars(['x_y','latlon_coord']),'y':x_y.loc[dict(x_y='y')].drop_vars(['x_y','latlon_coord'])})
    ...
    for year in inputs.years:
        for month in inputs.months:
            for dow in inputs.dows:
                ...
                if inputs.point_files:
                    frac1,frac2,x_y = calc_fracs(wrfchemi_ds=wrfchemi_ds,file=inputs.point_files[0].replace('[YYYY]',str(year)).replace('[YY]',str(year)[2:]).replace('[MM]',str(month).zfill(2)).replace('[DOW]',dow).replace('[dayhalf]','00z'))
                else:
                    frac1=None
                    frac2=None
                    x_y=None
