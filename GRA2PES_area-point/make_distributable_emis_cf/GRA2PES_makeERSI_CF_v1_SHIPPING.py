...
class inputs():
    ...
    gridded_files = ['/wrk/users/charkins/emissions/CAMSv4.2/regrid/2019_fixedOM/Month[MM]/ships_unmasked_[dayhalf].nc4']
    
    # CANNOT HAVE MORE THAN ONE CURRENTLY 
    point_files = []
    
    out_file1 = '/wrk/users/charkins/emissions/V7_GRA2PES/GRA2PESv1.0_ESRI/GRA2PESv1.0_SHIPPING/[YYYY][MM]/[DOW]/GRA2PESv1.0_SHIPPING_[YYYY][MM]_[DOW]_00to11Z.nc'
    out_file2 = '/wrk/users/charkins/emissions/V7_GRA2PES/GRA2PESv1.0_ESRI/GRA2PESv1.0_SHIPPING/[YYYY][MM]/[DOW]/GRA2PESv1.0_SHIPPING_[YYYY][MM]_[DOW]_12to23Z.nc'
    
    wrfchemi_example = '/wrk/charkins/emissions/total_emis/4km_domain/wrfinput_d01_conus4k'
    
    skip_methane_output = True
    
    sector_title_output = 'Shipping Emissions'
...
def place_vars(year,month,dow,out_ds_empty,frac1,frac2,x_y):
    ...
    for file in inputs.gridded_files + inputs.point_files:
        fn1 = file.replace('[YYYY]',str(year)).replace('[YY]',str(year)[2:]).replace('[MM]',str(month).zfill(2)).replace('[DOW]',dow).replace('[dayhalf]','00z')
        fn2 = file.replace('[YYYY]',str(year)).replace('[YY]',str(year)[2:]).replace('[MM]',str(month).zfill(2)).replace('[DOW]',dow).replace('[dayhalf]','12z')
    ...
    for c, variable in enumerate(out_nc_file.variables.keys()):
        ...
        if inputs.gridded_files:
            ...
            area_emis1 = sum_files_variable(inputs.gridded_files,year=year,month=month,dow=dow,dayhalf='00z',variable=variable)
            print('Making gridded dataset 12z: ')
            area_emis2 = sum_files_variable(inputs.gridded_files,year=year,month=month,dow=dow,dayhalf='12z',variable=variable)
