...
class inputs():
    ...
    out_dir               = '/wrk/users/charkins/emissions/CAMSv6.2/test/[YYYY]/Month[MM]/'
    out_filename_suffix   = ''
...
def make_emis(in_ds,regridder,out_example,times00z,times12z,CAMS_nc_struct,species_table,wrf_names,year,month,sector):
        ...
        for species_wrf in inputs.WRF_species:
            ...
            for species_cams in subtable.cams_name:
                ...
                if ind == 0:
                    ...
                    #elif not avg_cams:
                    sector_addition = xr.open_dataset(inputs.CAMS_dir + inputs.CAMS_filename_prefix + species_cams +inputs.CAMS_filename_suffix,
                                     chunks={'time': 1},cache=False)[sector][CAMS_time,:,:]#.squeeze(dim='time')
                    sector_addition.values = sector_addition.values * weight * unitconv
        ...
        if inputs.add_diurnal:
            if sector in inputs.sector_enable_DOW:
                if inputs.DOW == 'weekdy':
                    diurnal = pd.read_csv(inputs.input_dir + inputs.diurnal_fn_base + inputs.DOW_diurnal_suffix[DOW] + '.csv') # Import Diurnal Weight File
                    print('Diurnal file used: ' + inputs.input_dir + inputs.diurnal_fn_base + inputs.DOW_diurnal_suffix[DOW] + '.csv')
                    ...
                    print('Diurnal file 2 used: ' + inputs.input_dir + inputs.diurnal_fn_base + inputs.DOW_diurnal_suffix[DOW] + '.csv')
...
def main():
    ...
    CAMS_nc_struct = in_ds.isel(time=[249]).squeeze(dim='time',drop=True).drop(inputs.CAMS_sectors_full)# Make structure for rearranging all of the CAMS info
    ...
    for year in inputs.years:
        ...
        for month in inputs.months:
            ...
            for sector in inputs.CAMS_sectors_out:
                make_emis(in_ds,regridder,out_example,times00z,times12z,CAMS_nc_struct,species_table,wrf_names,year,month,sector)
